...
uint8_t Simulator::MenuSelect(int32_t newMouseX, int32_t newMouseY, uint8_t button) {
	...
	//lateeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeeer
	uint8_t gameData[] = {
	};
...
void Simulator::Dream(GraphicsContext* gc) {
	...
	if (ticks > 900) {
		...
		switch (this->ticks / (3600)) {
			...
			case 0:
				gc->FillBuffer(224, 44, 17, 19, cursorClassic, false);
				gc->FillBuffer(242, 44, 13, 20, LainPlatSprite1, true);
				break;
			case 1:
...
void Simulator::Fly(GraphicsContext* gc, uint16_t LainSpriteNum) { //<--------breaks memory
	
	//draw crying face
	switch (LainSpriteNum) {
	
		case 1:
			for (uint8_t i = 9; i < 14; i++) { gc->PutPixel(LainX+4, LainY+i, 0x2b); }
			for (uint8_t i = 9; i < 15; i++) { gc->PutPixel(LainX+9, LainY+i, 0x2b); }
			for (uint8_t i = 5; i < 9; i++) { gc->PutPixel(LainX+i, LainY+12, 0x37);}
			break;
		case 0:
			for (uint8_t i = 9; i < 15; i++) { gc->PutPixel(LainX+6, LainY+i, 0x2b); }
			for (uint8_t i = 9; i < 14; i++) { gc->PutPixel(LainX+11, LainY+i, 0x2b); }
			for (uint8_t i = 7; i < 11; i++) { gc->PutPixel(LainX+i, LainY+12, 0x37);}
			break;
		default:
			break;
	}
	...
	if ((LainY % 20) < 10) {
		...
		gc->FillBuffer(LainX-6, LainY-6, 9, 10, pigtailUp, false);
		gc->FillBuffer(LainX+13, LainY-6, 9, 10, pigtailUp, true);
	} else {
		gc->FillBuffer(LainX-7, LainY, 11, 8, pigtailDown, true);
		gc->FillBuffer(LainX+12, LainY, 11, 8, pigtailDown, false);
	}
...
void Simulator::Walk() {
	...
	if (mousePress && walkCycle < stepsNum) {	
		...
		LainX = walkPixels[walkIndex].x;
		LainY = walkPixels[walkIndex].y;
...
void Simulator::DrawRoom(GraphicsContext* gc) {
	...
	gc->FillPolygon(xRug2, yRug2, 4, 0x09);
	...
	gc->FillPolygon(xWall1, yWall2, 6, 0x09);
	...
	//posters
	gc->FillBuffer(208, 15, 20, 30, placeHolder1, false);
	gc->FillBuffer(267, 26, 20, 30, placeHolder2, false);
	gc->FillBuffer(9, 29, 20, 30, lunarPoster, true);
	...
	//tv stand
	gc->FillRectangle(141, 68, 38, 7, 0x39);
	...
	*/
	...
	if (this->sleeping) {
		...
		gc->FillBuffer(284, 82, 17, 10, LainSleep, false);
		...
		gc->FillBuffer(LainX, LainY, LainWidths[walkFrames], 49, LainSprites[walkFrames], facingLeft);

		switch (this->outfit) {
		
			case 1:
				gc->FillBuffer(LainX, LainY+18, LainWidths[walkFrames], 19, LainSummerSprites[walkFrames], facingLeft);
				break;
			default:
				break;
		}
