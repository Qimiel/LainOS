...
void Platformer::Draw(GraphicsContext* gc, uint16_t ticks) {
	...
	if (data.run) {
		...
		switch ((ticks/5)%2) {
			...
			case 1:
				gc->FillBuffer(playerX, playerY, 13, 20, LainPlatSprite1, data.facingLeft);
				break;
			default:
				gc->FillBuffer(playerX, playerY, 13, 20, LainPlatSprite2, data.facingLeft);
				break;
		}
	} else { gc->FillBuffer(playerX, playerY, 13, 20, LainPlatSprite1, data.facingLeft); }
}
