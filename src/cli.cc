...
void terminal(char* args, CommandLine* cli) {
	...
	if (cli->gui) { cli->appWindow->parent->CreateChild(1, "Terminal", 0); }
	else { cli->PrintCommand("This command is not available in text mode.\n"); }
}


void paint(char* args, CommandLine* cli) {
	
	if (cli->gui) { cli->appWindow->parent->CreateChild(2, "Paint", 0); }
	else { cli->PrintCommand("This command is not available in text mode.\n"); }
}

void Journal(char* args, CommandLine* cli) {
	
	if (cli->gui) { cli->appWindow->parent->CreateChild(3, "Notepad", 0); }
	else { cli->PrintCommand("This command is not available in text mode.\n"); }
}
...
void window(char* args, CommandLine* cli) {
	...
	if (cli->gui && cli->userWindow == nullptr) {
		...
		if (strlen(args) < 2) { cli->userWindow = cli->appWindow->parent->CreateChild(0, "Lain Window", cli); }
...
void help(char* args, CommandLine* cli) {
	...
	cli->PrintCommand("Ez does it ");
	cli->PrintCommand(ch);

	cli->PrintCommand("Try the following commands: \n\n");
	cli->PrintCommand("    'say (string)', 'Lain (int)', 'beep (int)',\n");
	...
	cli->PrintCommand("    ctrl+i - space mode (spacebar to shoot)\n");
	cli->PrintCommand("    ctrl+c - exit mode (return back to cli)\n");
}


void neofetch(char* args, CommandLine* cli) {
	...
	cli->PrintCommand("\n  F-E-T-C-H\n", w);
	cli->PrintCommand("          /\\  \n", p);
	cli->PrintCommand("         /--\\   ", p); cli->PrintCommand("OS:  ", b); cli->PrintCommand("Project\n", w);
	...
	cli->PrintCommand("  /----\\   //-----\\\n", p);
	cli->PrintCommand(" /------\\ //-------\\\n", p);
	cli->PrintCommand("/________\\/_________\\\n", p);
...
void Lain(char* args, CommandLine* cli) {
...
void CommandLine::hash_cli_init() {
	...
	this->hash_add("neofetch", neofetch);
...
char* CommandLine::command(char* cmd, uint8_t length) {
	...
	if (this->conditionIf == false && result != hash("fi")) { return "its 4am lol"; }
	//if (result != hash("else")) { return "its 4am lol"; }
	if (this->conditionLoop == false && result != hash("pool")) { return "its 4am lol"; }
