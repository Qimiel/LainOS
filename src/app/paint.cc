...
void BufferSwap(uint8_t* buf1, uint8_t* buf2, uint32_t size = GRAPHICS_LOGICAL_SIZE);
...
void Paint::ReadInput(char* file, CompositeWidget* widget, FileSystem* filesystem) {
	...
	uint16_t inputw = GRAPHICS_LOGICAL_WIDTH;
	uint8_t inputh = GRAPHICS_LOGICAL_HEIGHT;
...
void Paint::Zoom(CompositeWidget* widget, bool increase) {
	...
	for (uint32_t y = 0; y < GRAPHICS_LOGICAL_HEIGHT; y += zoomSize) {
		for (uint32_t x = 0; x < GRAPHICS_LOGICAL_WIDTH; x += zoomSize) {
			for (uint8_t i = 0; i < zoomSize*zoomSize; i++) {

				widget->PutPixel(x+(i%zoomSize), y+(i/zoomSize), this->backup[(GRAPHICS_LOGICAL_WIDTH*(y+menuY)+(x+menuX))/zoomSize]);
			}
		}
	}
}
...
void Paint::Bucket(int32_t x, int32_t y, CompositeWidget* widget) {
	...
	uint8_t color = widget->buf[LogicalIndex(x, y)];
	uint16_t xo = x;
	uint8_t yo = y;

	for (y = yo; widget->buf[LogicalIndex(xo, y)] != color; y++) {
		for (x = xo; widget->buf[LogicalIndex(x, y)] != color; x++) {
...
void Paint::Dimensions(bool width, bool increase) {
	...
	if (width) {
		if (increase) { this->width += 1 * (this->width < GRAPHICS_LOGICAL_WIDTH);
		} else { 	this->width -= 1 * (this->width > 10); }
	} else {
		if (increase) { this->height += 1 * (this->height < GRAPHICS_LOGICAL_HEIGHT);
